  ...
  return <>
    ...
    <section className="panel" aria-live="polite" aria-busy={state === "loading"}>
      ...
      {state === "ready" && run && <>
        ...
        {run.agent_kind === "project_watch" && <section aria-labelledby="project-watch-heading"><h2 id="project-watch-heading">Project Watch</h2>{!execution?.project_watch_result ? <p>No Project Watch result is available.</p> : <><p><strong>Status: {execution.project_watch_result.status === "changes_found" ? "Changes found" : "No meaningful change"}</strong></p><p>Window: {stamp(execution.project_watch_result.window_start)} to {stamp(execution.project_watch_result.window_end)}</p>{execution.project_watch_result.findings.length > 0 && <ol className="agent-list">{execution.project_watch_result.findings.map((finding, index) => <li key={index}><p>{finding.text}</p><p>Citations: {finding.citation_numbers.map((number) => `[${number}]`).join(" ")}</p></li>)}</ol>}{execution.project_watch_result.citations.length > 0 && <ol>{execution.project_watch_result.citations.map((citation) => <li key={citation.number}><code>{citation.entity_type} {citation.entity_id} · version {citation.version}</code></li>)}</ol>}</>}</section>}
