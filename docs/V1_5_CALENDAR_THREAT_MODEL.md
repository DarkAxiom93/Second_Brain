...
| G13 Credential revocation/replacement races | Serialize/fence envelope generation and account revision; install/reauthorize only after complete two-scope and ID-token identity validation; stale refresh cannot overwrite; revoke blocks before requests and deletes the exact envelope. | Revoke vs refresh/request, two refreshes, token rotation, concurrent reauthorization with different `sub`, failed replacement preserving the prior envelope, missing/locked store, delete failure, revoked/expired token and stale worker result. |
