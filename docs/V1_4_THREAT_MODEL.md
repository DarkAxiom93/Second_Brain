...
| C02 Excessive API scopes | Reads data beyond operator intent | Fine-grained expiring token; exact read-only permission allowlist; validate granted-scope fingerprint and selected repositories before every sync | Reject/disable on missing, unknown, write, or expanded permission; never continue with a superset | Read-only minimum accepted; write/admin/org/user/email scopes rejected; scope changes between pages and after replacement |
...
| C06 Stale/revoked/expired credential | Unauthorized attempts, confusing stale data, repeated failures | Validate reference, expiry, account fingerprint, and granted scopes; explicit replacement/revocation; no automatic refresh | Disable/pause reads, keep last snapshots marked stale, one safe notice; require operator action | Missing/locked store, expiry at claim/request/page boundary, server revocation, local delete failure, replacement race, identity-changing token |
